...
function TasksKanbanBoard() {
    ...
    const onDragEnd = async ({ source, destination }) => {
        ...
        const movedTask = { ...sourceCol.tasks[source.index] }; // copie de la tâcheAdd commentMore actions
        ...
        // 🔁 Mise à jour backendAdd commentMore actions
    ...
    // const getPriorityColor = (priority) => {Add commentMore actions
    //         switch (priority) {
                ...
                <div className="flex gap-4 p-4 overflow-auto">
                    {Object.entries(columns).map(([status, column]) => (
                        < Droppable droppableId={status} key={status} >
                            {(provided) => (
                                <div className="bg-gray-100 rounded-lg p-3 w-64 min-h-[300px]"
